    ...
    if "success" in res:
        try:
            ...
            try:
                ...
                record_edit_event(conn, workspace=ctx.workspace, file_path=file_path, before_content=before_content, after_content=after_content, session_id=ctx.session_id, event_source="cortex_mcp", tool_name="pc_strict_replace", edit_summary=f"Strict edit: {file_path}")
            finally:
                conn.close()
        except Exception as e:
            res["event_log_error"] = str(e)

        hook_feedback = dispatch(ctx.workspace, "after_edit", os.path.join(ctx.workspace, file_path))
        if hook_feedback:
        ...
        dispatch(ctx.workspace, "after_save_observation")
    return res
