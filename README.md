...
최근 **v2.1.0 릴리즈**부터는 "추론 경제(Inference Economy)" 철학을 바탕으로, 무거운 데몬 대신 백그라운드 오버헤드가 제로에 가까운 **하드웨어 인지형 하이브리드 엔진**으로 거듭났습니다.
...
*   **기회적 인덱싱 (Opportunistic Indexing)**: 상시 구동되는 리소스 낭비형 데몬을 제거하고, 에이전트가 MCP를 호출하는 순간 변경분만 추적하여 오버헤드를 없앴습니다.
