# Claude Agent Operating Guide (v4.2)
...
- **Proactive Discovery**: 세션이 시작되면 `python3 ./.agents/scripts/cortex/cortex_ctl.py status`를 통해 인프라 가동 상태를 확인하고 필요한 지식을 확보하십시오. **(인프라는 MCP 서버에 의해 자동 기동되므로, 미가동 시에만 'start'를 수행하여 리소스를 최적화하십시오.)** 이후 `mcp__cortex-mcp__pc_capsule`을 호출하여 `.agents/tasks/` 내의 진행 상황과 현재 Lane의 상태를 보고하되, **기술적 지식 확보(Retrieval)는 `mcp__cortex-mcp__pc_memory_search_knowledge`를 우선 순위로 사용하십시오.**
- **Guardrails** (도구 통제): 파일/콘텐츠 검색 시 grep, find 등 내장 도구 및 쉘 명령어를 직접 실행하는 것을 금지하며, 반드시 mcp_cortex 파이프라인(pc_capsule 등)을 사용하십시오. **파괴적 명령어 실행 지침은 먼저 mcp__cortex-mcp__pc_memory_read로 rule::guardrails를 조회해 안전 정책을 확인하십시오.** 또한 고립된 지식(React, Skill 등) 탐색 시에는 **`mcp__cortex-mcp__pc_memory_search_knowledge`**에 `category: skill` 필터를 명시하십시오.
...
- **아키텍처 및 지식 탐색**: 프로젝트 지형 파악에는 `mcp__cortex-mcp__pc_capsule`을 사용하고, **정예 스킬 및 독립 지침 확보에는 `mcp__cortex-mcp__pc_memory_search_knowledge`를 활용하십시오.** 
- **아키텍처 변경**: 엔진 코어나 파서 추가 시 먼저 Cortex 엔진을 가동하여 **MCP 도구(`mcp__cortex-mcp__pc_memory_read`)**로 `rule::architecture` 지식을 조회하고 Strategy Pattern과 훅(Hooks) 규칙을 준수하십시오.
